...
def process_file(file_path):
    ...
    # Limpa completamente o contexto anterior (léxico)
    symbol_table = []
    token_count = {token: 0 for token in tokens}
    processed_tokens = []
    error_tokens = []
    last_keyword = None  # Reseta a última palavra-chave
    # Reinicia o estado sintático
    sintatico.reset_syntax_state()
    ...
    try:
        ...
        # Após o processamento léxico, executa a análise sintática no módulo dedicado
        sintatico.analyze_syntax(processed_tokens)
...
# ===================== Proxies para o módulo sintático =====================

def show_syntax_summary():
    """Proxy para mostrar a síntese da análise sintática."""
    sintatico.show_syntax_summary()


def show_ontology_errors():
    """Proxy para mostrar os erros da ontologia, incluindo erros léxicos."""
    sintatico.show_ontology_errors(error_tokens)